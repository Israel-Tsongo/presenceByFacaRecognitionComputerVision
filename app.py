...
class VideoCamera(object):
    ...
    path = 'D:\django\Flask-Based-Face-Recognition-Attendance-System-master\static\images'
    images = []
...
def markAttendance(name):
    ...
    global filename
    print(filename)
    with open('C:/xampp/htdocs/attendances/'+filename, 'r+') as f:
...
def forgetpassword():
    if request.method == "POST":
        ...
        if email==m1:
            msg = Message('Hello', sender='esai@gmail.com', recipients=[m1])
    ...
    if 'loggedin' in session:
        if session.get('role')=='admin':
            if request.method == 'POST':
                ...
                img.save(os.path.join('D:\django\Flask-Based-Face-Recognition-Attendance-System-master\static\images',img.filename))
                os.rename('D:\\django\\Flask-Based-Face-Recognition-Attendance-System-master\\static\images'+'\\'+img.filename, 'D:\\django\\Flask-Based-Face-Recognition-Attendance-System-master\\static\\images\\'+username+'.jpg')
                password=request.form['password']
                role='student'
                cur = mysql.connection.cursor()
                cur.execute("""
...
def delete(username):
    if 'loggedin' in session:
        if session.get('role')=='admin':
            ...
            mysql.connection.commit()
            os.remove('D:\django\Flask-Based-Face-Recognition-Attendance-System-master\static\images'+username+'.jpg')
    ...
    if 'loggedin' in session:
        ...
        if session.get('role')=='faculty':
            if request.method == 'POST':
                ...
                global myurl
                myurl='http://localhost/attendances/'+fname
                print(myurl)
...
def video_feed():
    if 'loggedin' in session:
        if session.get('role')=='faculty':
            ...
            with open('C:/xampp/htdocs/attendances/' + filename, 'w') as f:
                pass
